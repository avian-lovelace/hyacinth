...
testLexer = do
  describe "lexText" $ do
    describe "Keywords" $ do
      it "lexes type" $ let
        input = "type"
        expected = fromList [Token TypeToken (startRange 4)]
        in lexText input `shouldBe` (Success expected)

      it "lexes let" $ let
        input = "let"
        expected = fromList [Token LetToken (startRange 3)]
        in lexText input `shouldBe` (Success expected)
    ...
    describe "Char literals" $ do
      ...
      it "throws UnterminatedCharError on '" $ let
        input = "'"
        expected = UnterminatedCharError initPosition
        in lexText input `shouldBe` (Error expected)
      
      it "throws UnterminatedCharError on 'a" $ let
        input = "'a"
        expected = UnterminatedCharError initPosition
        in lexText input `shouldBe` (Error expected)

      it "throws InvalidCharLiteralError on ''" $ let
        input = "''"
        expected = InvalidCharLiteralError initPosition
        in lexText input `shouldBe` (Error expected)
      
      it "throws InvalidCharLiteralError on 'ab'" $ let
        input = "'ab'"
        expected = InvalidCharLiteralError initPosition
        in lexText input `shouldBe` (Error expected)
