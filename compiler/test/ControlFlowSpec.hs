...
testControlFlow = do
  ...
  describe "If expression errors" $ do
    it "The condition of an if expression must have type boolean" $
      "print if 5 then \"foo\" else \"bar\";" `failsToCompileWithError` ifThenElseExpressionConditionTypeError
  ...
  describe "While loop errors" $ do
    it "The condition of an if expression must have type boolean" $
      "while \"foo\" loop { print 1; };" `failsToCompileWithError` whileLoopConditionTypeError
